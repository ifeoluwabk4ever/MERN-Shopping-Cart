...
const EditProduct = ({ categories, isLoading, getCategories, allProduct }) => {

   const [images, setImages] = useState(true)
   let [loading, setLoading] = useState(false)
   let [loading2, setLoading2] = useState(false)
   let [editProduct, setEditProduct] = useState([...allProduct])
   const [detailProduct, setDetailProduct] = useState([])
   const params = useParams()
   let { company, uses, product_name, price, dosage, side_effects, description, category, product_slug, product_image } = detailProduct
   ...
   let handleUploadImage = async e => {
      ...
      try {
         ...
         setDetailProduct({ ...detailProduct, product_image: res.data.msg.product_image })
      } catch (error) {
   ...
   let handleDestroyImage = async () => {
      try {
         ...
         setDetailProduct({ ...detailProduct, product_image: '' })
      } catch (error) {
         toast.error(error.response.data.msg)
      }
   }
   ...
   let handleFileChange = async e => {
      ...
      try {
         ...
         let res = await axios.put(`/api/products/${product_slug}`, productData)
         setEditProduct([res.data, ...editProduct])
   ...
   const handleDataChange = input => e => {
      setDetailProduct({ ...detailProduct, [input]: e.target.value })
   }
   ...
   return (
      <Container className="my-4">
         <h1 className="text-center text-uppercase my-3">Edit <span className="title-color"> {product_name}</span> detail</h1>
            ...
            <form onSubmit={handleFileChange}>
               <Card className="shadow">
                  ...
                  <CardBody>
                     ...
                     {
                        !loading2 &&
                        <button type="submit" className="btn btn-primary text-capitalize">Update product</button>
                     }
                  </CardBody>
               </Card>
            </form>
...
export default connect(mapStateToProps, { getCategories })(EditProduct)